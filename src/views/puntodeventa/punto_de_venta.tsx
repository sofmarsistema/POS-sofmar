...
export default function PuntoDeVenta() {
  ...
  useEffect(() => {
    ...
    const fetchVendedores = async () => {
      ...
      try {
        ...
        console.log( 'los vendedores son',response.data.body);
      } catch (err) {
        if (err instanceof Error) {
          setError(err.message);
        } else {
  ...
  const ventaData = {
    ...
    detalle_ventas: items.map((item) => {
      ...
      const deve_exentas = Math.max(
        item.exentas * item.cantidad - itemDescuento,
        0
      );
      const deve_cinco = Math.max(
        item.impuesto5 * item.cantidad - itemDescuento,
        0
      );
      const deve_diez = Math.max(
        item.impuesto10 * item.cantidad - itemDescuento,
        0
      );
  ...
  const handleSelectPresupuesto = async (presupuesto: Presupuesto) => {
    try {
      const response = await axios.get(`${api_url}presupuestos/detalles/?cod=${presupuesto.codigo}`);
      ...
      <ChakraProvider>
        ...
        >
          ...
          >
            ...
            <Button leftIcon={<FileText />} ml="auto" onClick={handleOpenPresupuestoModal}>
          ...
          <Flex flexDirection={isMobile ? "column" : "row"}>
                  ...
                  <Tbody>
                    {items.map((item, index) => (
                      <Tr key={index}>
                        ...
                        <Td isNumeric>
                          {formatCurrency(
                            actualizarMoneda(item.exentas) * item.cantidad
                          )}
                        </Td>
                        <Td isNumeric>
                          {formatCurrency(
                            actualizarMoneda(item.impuesto5) * item.cantidad
                          )}
                        </Td>
                        <Td isNumeric>
                          {formatCurrency(
                            actualizarMoneda(item.impuesto10) * item.cantidad
                          )}
                        </Td>
                        <Td>
                          <Button
            ...
            >
              ...
              <Box pt={2}>
                ...
                <Divider borderWidth={"2px"} borderColor={"blue.500"} my={1} />
                <Text fontSize="sm" fontWeight="bold">
                  Total IVA 10%: {formatCurrency(calcularTotal10() / 10)}
                </Text>
                <Divider borderWidth={"2px"} borderColor={"blue.500"} my={1} />
                <Text fontSize="sms" fontWeight="bold">
        ...
        <VentaModal
              ...
              ventaId={newSaleID}
            />
        <Modal isOpen={isPresupuestoModalOpen} onClose={handleClosePresupuestoModal} size="full">
          <ModalOverlay />
          <ModalContent>
            <ModalHeader>Consulta de Presupuestos</ModalHeader>
            <ModalCloseButton />
            <ModalBody>
              <ConsultaPresupuestos onSelectPresupuesto={handleSelectPresupuesto} onClose={handleClosePresupuestoModal} isModal={true} />
            </ModalBody>
          </ModalContent>
        </Modal>
      </ChakraProvider>
